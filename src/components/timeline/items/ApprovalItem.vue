<script setup lang="ts">
    import { Button, Card, Tag, useToast } from "primevue";
    ...
    <Card :pt="{
        ...
        <template #title>
            <div class="justify-between flex items-center">
                <div class="text-sm">Created {{ formatRelativeTime(item.submission_date) }}</div>
        ...
        <template #content>
            <div>
                <div v-dompurify-html="item.submission_comment"></div>
            </div>
        </template>
        <template #footer>
            <Form v-if="canAnswer && item.status < 3" ref="approval_answer_form">
