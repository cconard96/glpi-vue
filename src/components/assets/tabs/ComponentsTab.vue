<script setup lang="ts">
    import {DataTable, Card, Column} from 'primevue';
    ...
    const nonempty_components = computed(() => {
        if (components_info.value === null) {
            return {};
        }
        const result = {};
        for (const [key, items] of Object.entries(components_info.value)) {
            if (items.length > 0) {
                result[key] = items;
            }
        }
    ...
    onMounted(async () => {
        await doGraphQLRequest(`query {
            ...
            HardDriveItem(filter: "itemtype==${props.main_itemtype_model.getOpenAPISchemaName()};items_id==${props.items_id}") {
                id itemtype items_id hard_drive { designation manufacturer { id name } model { id name } rpm cache capacity_default interface { id name } } serial otherserial busID capacity
            }
            MemoryItem(filter: "itemtype==${props.main_itemtype_model.getOpenAPISchemaName()};items_id==${props.items_id}") {
                id itemtype items_id memory { designation manufacturer { id name } model { id name } frequency size_default } serial otherserial busID size
            }
            ...
            ProcessorItem(filter: "itemtype==${props.main_itemtype_model.getOpenAPISchemaName()};items_id==${props.items_id}") {
                id itemtype items_id processor { designation manufacturer { id name } model { id name } frequency frequency_default nbcores_default nbthreads_default } serial otherserial busID frequency nbcores nbthreads
            }
            ...
            components_info.value = res.data.data;
        });
    });
...
<template>
    <section v-if="components_info !== null" class="">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card class="flex flex-row items-center" :pt="{body: { class: 'p-2'} }">
                <template #header>
        ...
        <div class="text-4xl text-center mb-2">Components</div>
        <div class="overflow-y-auto">
            <div v-for="(items, component_type) in nonempty_components" :key="component_type" class="mb-6">
                <DataTable :value="items" :rows="100" :totalRecords="items.length">
                    <template #header>
