<script setup lang="ts">
    ...
    import { Button, Checkbox, InputNumber, InputText, SelectButton, Textarea, useToast, Fieldset } from "@cjdevstudios/bumblevue";
    ...
    const mainItem: ReturnType<typeof useAsset<'Monitor'>> = inject('mainItem');
...
<template>
    <section class="h-full overflow-y-auto">
        <AdvancedForm schemaName="Monitor" :initialValues="initialValues" class="flex flex-col gap-4 w-full sm-w-56 px-4" @submit="onFormSubmit">
            <FormFields>
                ...
                <ValidatedFormField name="size" label="Size" :as="InputNumber" :fieldProps="{min: 0, minFractionDigits: 0, maxFractionDigits: 2}"></ValidatedFormField>
            ...
            <Fieldset legend="Ports / Capabilities">
                <FormFields>
                    <ValidatedFormField name="has_microphone" label="Microphone" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_speakers" label="Speakers" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_subd" label="Sub-D" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_bnc" label="BNC" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_dvi" label="DVI" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_hdmi" label="HDMI" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_displayport" label="DisplayPort" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                    <ValidatedFormField name="has_pivot" label="Speakers" :as="Checkbox" :fieldProps="{binary: true}"></ValidatedFormField>
                </FormFields>
            </Fieldset>
